...
// Motor de ASTEROIDS portado a TypeScript de
// references/started-games/02-asteroids/game.js (recuperado con
// `git show HEAD:references/started-games/02-asteroids/game.js`).
//
// Misma física, colisiones, partición de asteroides, power-up de disparo
// triple, wrap toroidal e invencibilidad al reaparecer que el original.
// Sin `window`/`document`/`canvas` globales de nivel de módulo: el
// `CanvasRenderingContext2D` y el estado de input se inyectan, para que
...
export interface AsteroidsInputState {
  left: boolean;
  right: boolean;
  up: boolean;
  /** true solo en el frame en que se presiona espacio (equivalente a pressed('Space') del original). */
  shoot: boolean;
}
...
class Ship {
  ...
  update(dt: number, input: AsteroidsInputState) {
    if (this.dead) return;
...
export function createEngine(ctx: CanvasRenderingContext2D) {
  ...
  // ── Update ──────────────────────────────────────────────────────────────────
  function update(dt: number, input: AsteroidsInputState) {
    if (state === "gameover") {
